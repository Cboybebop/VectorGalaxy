...
function getKvConfig() {
  ...
  const token =
    ...
    process.env.KV_REST_API_TOKEN_READ_WRITE ||
    process.env.KV_REST_API_TOKEN_READ_WRITE_TOKEN;
...
export default async function handler(req) {
  ...
  if (!url || !token) {
    return json(
      {
        error:
          'Vercel KV is not configured. Add KV_REST_API_URL and one of KV_REST_API_TOKEN / KV_REST_API_TOKEN_READ_WRITE / KV_REST_API_TOKEN_READ_WRITE_TOKEN.'
      },
      503,
      origin
    );
